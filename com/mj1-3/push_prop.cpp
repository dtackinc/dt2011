...
dtmj_push_prop_c::CreateInstance(
  ...
  HRESULT *phr)
{
  CUnknown *punk = new dtmj_push_prop_c(lpunk, phr);
  if (punk == NULL) 
    *phr = E_OUTOFMEMORY;
  return punk;
...
// Constructor
//
dtmj_push_prop_c::dtmj_push_prop_c(
  LPUNKNOWN pUnk, 
  HRESULT *phr) :
...
//
BOOL dtmj_push_prop_c::OnReceiveMessage(HWND hwnd,
                                          UINT uMsg,
                                          WPARAM wParam,
                                          LPARAM lParam)
{
...
//
HRESULT dtmj_push_prop_c::OnConnect(IUnknown *pUnknown)
{
...
//
HRESULT dtmj_push_prop_c::OnDisconnect()
{
  if (this->prop_i == NULL) 
    return E_UNEXPECTED;
  this->prop_i->Release();
...
//
HRESULT dtmj_push_prop_c::OnActivate()
{
  Edit_SetText(GetDlgItem(m_Dlg, IDC_DGSPEC), this->prop.dgspec);
...
//
HRESULT dtmj_push_prop_c::OnDeactivate(void)
{
  ASSERT(this->prop_i);
  this->prop_i->get_prop(&this->prop);
...
//
HRESULT dtmj_push_prop_c::OnApplyChanges()
{
  DTAX_F("dtmj_push_prop_c::OnApplyChanges");
  ASSERT(this->prop_i);
