...
CFactoryTemplate g_Templates[] = {
  {
    L"Dtack Mjpeg encode1-4 transform filter", 
    &dtmj_encode_guid,
    dtmj_encode_c::CreateInstance
  },
  {
    L"Dtack Mjpeg encode1-4 transform filter property page", 
...
CPersistStream(punk, phr)
{
  ...
  this->fb_brightness = 0;
  this->fb_contrast = 0;
  this->fb_hue = 0;
  this->fb_panx = 0;
  this->fb_pany = 0;
  this->fb_quality = 0;
  this->fb_saturation = 0;
  this->fb_source = 0;
  this->fb_zoom = 0;
}
  ...
  CMediaType *omedia)
{
  ...
  format->fb_brightness = this->fb_brightness; 	// update other stuff we have
  format->fb_contrast = this->fb_contrast;
  format->fb_hue = this->fb_hue;
  format->fb_panx = this->fb_panx;
  format->fb_pany = this->fb_pany;
  format->fb_quality = this->fb_quality;
  format->fb_saturation = this->fb_saturation;
  format->fb_source = this->fb_source;
  format->fb_zoom = this->fb_zoom;
...
#define PROCESS(CMD) \
  if (DT_ABBREV(command, #CMD ",")) \
  { \
    int fb_ ## CMD = atoi(command + strlen(#CMD ",")); \
    dt_dbg(ctl, F, DT_DBG_MASK_COMMAND, \
      "%s %d->%d", #CMD,  \
      this->fb_ ## CMD, fb_ ## CMD); \
    if (this->fb_ ## CMD != fb_ ## CMD) \
      changed++; \
    this->fb_ ## CMD = fb_ ## CMD; \
    DTAX_Q(dtmj_command_passthrough,( \
      ctl, this, command)); \
  }
