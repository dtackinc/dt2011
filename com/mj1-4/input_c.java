...
extends Thread
{
  ...
  run()                                 /* run the loading loop             */
  {
    ...
    try 
    {
	  ...
	  int parts = this.master(stream);	// read master header
	  if (parts != 0)					// understood the mime type?
      do {
		this.set_state("loading");
		int l;
		if (parts == -1)				// multipart mime?
		{
		  l = this.header(stream);		/* read the multipart header 		*/
		  if (l == 0)					/* bad header? 						*/
		    break;
		}
		else
  ...
  throws IOException
  {
	...
	if (this.content_type.startsWith(	/* multipart?						*/
        "multipart/x-mixed-replace"))
	  return -1;

	if (this.content_type.startsWith(	/* single part?						*/
        "image/jpeg"))
	  return l;
