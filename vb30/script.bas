...
Sub script_next(f As scriptform)
  ...
  do
    ...
    if err <> 0 then
      s = f.scriptname
      call error_notify("Sorry, read error <" + error$ + "> in script " + s + ".")
      call script_finished(f)
      exit sub
    end if
    ...
    if rc <> RC_GOOD then
      f.done.additem "MISS " + f.executable.caption
    end if
...
'
Sub script_run (f As scriptform, scriptname as string, env as string, outfile as string)
  dim fd as integer
  fd = FreeFile()

  dim s as string
  s = profile_get(PROFILE_INI_MASTER, PROFILE_APP_GLOBALS, PROFILE_KEY_SCRIPTPATH)
  if s <> "" then
    s = s + "\"
  end if
  ...
  if err <> 0 then
    call error_notify("Sorry, could not open <" + s + ">")
    call script_finished(f)
    exit sub
  end if
