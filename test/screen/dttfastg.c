...
putvirt(								/* copy rows as virtual buffer		*/
  ...
  int h)
{
  DT_F("dttfastg_putvirt");
  int handle = -1;
  int fgrc = -1;
  
  handle = fg_vbdefine(data, w, h);		/* virtual buffer around our data 	*/
  ...
  fg_vbpaste(0, w-1, 0, h-1, x0, y0+h-1);	/* virtual buffer to active page */
  ...
  int h)
{
  ...
  fg_vbcut(0, w-1, 0, h-1, x0, y0+h-1);	/* active page to virtual buffer	*/
...
DTT_MAIN(dttfastg)
{
  ...
  int nopause;
  dtkeybd_t keyboard;
  void (*put)(char *, int, int, int, int) = NULL;
  void (*get)(char *, int, int, int, int) = NULL;
