...
# if DTPRJ_BTTV == DT_1
  ...
  {
    priv->interlace = !!strstr(p, "interlace");
  ...
  wtw.width = fb->xe;
  wtw.height = fb->ye;
  wtw.height *= (priv->skip + 1);
  wtw.height /= (priv->dup + 1);
  wtw.swidth = wtw.width;
  wtw.sheight = wtw.height;
  wtw.norm = 1;							/* ntsc 							*/
  wtw.interlace = priv->interlace;
  ...
  priv->mem_size = fb->xe * 3 * fb->ye;
										/* ................................ */
  fb->priv = priv;
  DT_C(dtfb_bt_report,(ctl, fb, "1"));	/* report what we have so far 		*/
