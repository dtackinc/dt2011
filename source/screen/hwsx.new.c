...
typedef struct {
  ...
  int bits;								/* depth of pixmap as last written  */
...
static void xioerror_handler(
  Display *display)
{
  xerror = -1;							/* just save error in global	    */
}
  ...
  int bits)
{
  ...
  else
  {
    ...
    XQueryColors(priv->display,			/* get root's colors			    */
      DefaultColormap(priv->display,
        priv->screen_num),
      xcolor, 256);

    XStoreColors(priv->display,			/* set this window's colors		    */
      priv_win->colormap,
      xcolor, 256);
  ...
  {
    XSetWindowColormap(					/* why do this after CreateWindow?	*/
      priv->display,
      priv_win->window,
      priv_win->colormap);
  }
  ...
  XSetWMNormalHints(priv->display,
    priv_win->window,
    normal_hints);

  XFree((FREE_CAST)normal_hints);
										/* ................................ */
  ...
  XStringListToTextProperty(			/* create a text property			*/
    &tp, 1, &text);
  
  XSetWMName(priv->display,				/* name goes in title bar			*/
    priv_win->window,
    &text);
  XSetWMIconName(priv->display,			/* name goes in icon				*/
    priv_win->window, &text);

  XFree((FREE_CAST)text.value);

  class_hints.res_name =				/* window instance name				*/
    priv_win->name;
  class_hints.res_class =				/* window class name				*/
    "dt";
  XSetClassHint(priv->display,			/* set window name information		*/
  ...
  if (priv_win->gc == 0)
    return dt_err(ctl, F,
      "at XCreateGC");
  XSetFillStyle(priv->display,			/* only fill action is to clear	    */
    priv_win->gc,
    FillSolid);

										/* ................................ */
  XSelectInput(							/* select events we want to see		*/
    priv->display,
    priv_win->window,
    ExposureMask |
    KeyPressMask |
    ...
    StructureNotifyMask);				/* window moved or resized		    */
										...
										/* ................................ */
  XMapWindow(priv->display,				/* pop up the window				*/
    priv_win->window);
  ...
  hws_win->ye = ye;
  priv_win->bits = bits;				/* remember how deep pixmap is	    */
  ...
  dt_event_t *event)
{
  ...
  do {
	...
	{
      XNextEvent(priv->display,			/* grab the next pending event 		*/
        &report);
	}
										/* ................................ */
	else
    ...
    if (report.type == MotionNotify)	/* motion event? 					*/
    {
      ...
      if (report.xmotion.is_hint)		/* just a motion hint? 				*/
	  {
        Window root, child;
        int rootx, rooty;
        XQueryPointer(					/* go get pointer position 			*/
          priv->display,
          window,
          &root, &child, 
          &rootx, &rooty,
        ...
        report.type == KeyRelease)
    {
      ...
      XLookupString(&report.xkey,		/* get keysym and string		    */
    ...
    priv_win = (dthws_x_priv_win_t *)hws_win->priv;			/* our private info on the window 	*/
...
#       endif
          XPutImage(priv->display,		/* replace the obstructed part	    */
            priv_win->window,
            priv_win->gc,
            priv_win->image,
            report.xexpose.x,
...
# endif
  ...
  if (priv_win->image != NULL)
  {
    ...
    XDestroyImage(priv_win->image);	/* let X free its image structure	*/
    priv_win->image = NULL;
  }
  ...
  {
    XFreeCursor(priv->display,
      priv_win->cursor);
    priv_win->cursor = 0;
  }

  XFreeGC(priv->display,
    priv_win->gc);

  XDestroyWindow(priv->display,		/* delete window					*/
    priv_win->window);

  XFlush(priv->display);				/* force deletion now				*/
  ...
  int count)
{
  ...
  { 
	...
	XStoreColors(						/* force root's colormap like ours	*/
      priv->display,
      DefaultColormap(
        priv->display,
        priv->screen_num), 
	...
	XFreeColors(						/* once we set them we're done	    */
      priv->display,
      DefaultColormap(
        priv->display,
        priv->screen_num), 
  ...
  else									/* window has writable colormap? 	*/
  {
    ...
    XStoreColors(						/* load cells in window's colormap	*/
      priv->display,
      priv_win->colormap,
      xcolor, count);
...
# endif
  {
    XPutImage(priv->display,			/* update patch on screen		    */
      window,
      priv_win->gc,
      priv_win->image,
      x0, y0,
...
# ifndef NOMBUF
  if (priv->use_multibuf)
  { 
	XmbufDisplayBuffers(				/* display updated buffer 			*/
  ...
  int count)
{
  ...
  XQueryColors(priv->display,			/* get colors					    */
  ...
  int background)
{
  ...
  XSetForeground(priv->display,			/* filling clears to background	    */
    priv_win->gc,
    background);

  XFillRectangle(priv->display,
    priv_win->window,
    priv_win->gc,
  ...
  dthws_win_t *hws_win)
{
  ...
  XRaiseWindow(							/* tell X to raise window			*/
    priv->display,
    priv_win->window);
  ...
  dthws_win_t *hws_win)
{
  ...
  XLowerWindow(							/* tell X to lower window			*/
    priv->display,
    priv_win->window);
  ...
  int op)
{
  ...
  if (priv_win->cursor == 0)			/* haven't made our cursor yet? 	*/
  {
    ...
    XFreePixmap(priv->display, pixmap);	/* free pixmap resource			    */

	XDefineCursor(priv->display,		/* associate cursor with window 	*/
      priv_win->window,
      priv_win->cursor);
  }
  ...
  int y)
{
  ...
  XMoveWindow(priv->display,			/* move window 						*/
    priv_win->window, 
  ...
  {
    XWarpPointer(priv->display,			/* pointer follows 					*/
      priv->rootwin,
