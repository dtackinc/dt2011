...
u_rc_e 
U_API
umplay_op_control_transact(
  dtparam_t *param,
  const char *command,
  char *reply,
  int maxreply)
{
  U_F("umplay_op_control_transact");
  ...
  U_C(dtipc_find,(ctl, ipc,				/* contact server 					*/
    param->connect, 0, 0, -1));
  connected = 1;
  U_C(dtipc_write_all,(ctl, ipc,		/* write command 					*/
    command, strlen(command)));
  U_C(dtipc_write_all,(ctl, ipc,		/* command termination 				*/
    "\n", 1));	
  U_C(dtipc_read_line,(ctl, ipc,		/* read response 					*/
    reply, maxreply));
  reply[strlen(reply)-1] = '\0';		/* nuke the newline 				*/
  U_C(dtipc_write_all,(ctl, ipc,		/* transaction termination			*/
    "\n", 1));	
...
u_rc_e 
U_API
umplay_op_control_keybd(
  dtparam_t *param)
{
  U_F("umplay_op_control_keybd");
  u_ctl_t *ctl = &param->ctl;
  dtkeybd_t keybd;
  ...
  U_C(umplay_command_prompt,(ctl,		/* make prompt 						*/
  ...
  while(1)
  {
	...
	if (key == '.')						/* quit session? 					*/
	  break;
	U_C(umplay_command_string_from_key,(	/* look up command from key 		*/
      ctl, key, &string,
      prompt, sizeof(prompt)));

	if (key == UMPLAY_KEY_SOURCE)		/* special case: get filename 		*/
	{
	  dtstr_printf(ctl,
	  ...
	  dtstr_printf(ctl,
        command, sizeof(command),
        "%s ", string);
	  U_Q(umplay_keybd_line,(ctl,		/* get filename from console 		*/
        &keybd,
        command+strlen(command),
	...
	if (string != NULL)					/* good command? 					*/
  	  U_C(umplay_op_control_transact,(	/* send command 					*/
        param, string,
        prompt, sizeof(prompt)));
  ...
  dtparam_t *param)
{
  ...
  {
	char response[256];
	U_Q(umplay_op_control_transact,(	/* send command 					*/
      param, command,
