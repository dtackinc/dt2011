...
//
// Here is the code for the decoder which gets the prediction error, recovers the
// pel value, and propagates zeros down the binary tree following a leaf codeword.
//
#if defined(DIAMOND_PROP_LEFT)||defined(DIAMOND_PROP_RIGHT)||defined(SQUARE_PROP)
#ifdef SQUARE_PROP
...
#endif
#ifdef DIAMOND_PROP_RIGHT
		if ((level == 1)&&(!error)) // Sibling was 0 on last band
			error = codein[4].get();
		else
#endif
                error = codein[BASE+ZEROTREE].get();
		if (error < 130)
		    ; // Skip tests
                else if (error == 130) {
		    error = codein[BASE+1].get();
		    PROPPEL = 1;
                    }
		else if (error == 131) {
		    error = 0;
			...
			// Actually this can only happen when onestep is true
			// but done here for convenience
			if (!codein[0].getbit())
				{
				error = 130;
				}
			else if (!codein[0].getbit())
				{
				error = 131;
				}
			}
...
#if defined(DIAMOND_PROP_LEFT) || defined(DIAMOND_PROP_RIGHT)
		if (level > 1)
			PROPPEL = 1;
#endif
		error = 0;	// Scratch set - error is zero
		}
	    if (estimate < 0)
		{
		fastpred = 0;
