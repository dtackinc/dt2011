...
DT_RCSID("fourth/mpegssg $RCSfile: store.c,v $ $Revision: 1.7 $");
#include <umdec.h>
...
/* private prototypes */
static int store_one _ANSI_ARGS_((char *outname, unsigned char *src[],
...
int frame;
{
  ...
  if (progressive_sequence || progressive_frame || Frame_Store_Flag)
  {
    ...
    rc = store_one(outname,src,0,Coded_Picture_Width,vertical_size);
  }
  else
  {
    /* interlaced */
    sprintf(outname,Output_Picture_Filename,frame,'a');
    rc = store_one(outname,src,0,Coded_Picture_Width<<1,vertical_size>>1);

    sprintf(outname,Output_Picture_Filename,frame,'b');
    rc = store_one(outname,src,
 ...
 */
static int store_one(outname,src,offset,incr,height)
char *outname;
unsigned char *src[];
int offset, incr, height;
{
  ...
  if (ssg_umdec->control.mode ==		/* we are supposed to be stopped? 	*/
	  UMDEC_MODE_STOP)
    return 0;
  ...
  case T_Y_CB_CR:
	...
	{
	  dt_err(ctl, F, "chroma format %d, but need %d",
        chroma_format, CHROMA420);
	  rc = 0;
	}
	else
      rc = ssg_y_cb_cr((void **)src, offset, incr, horizontal_size, height);
    break;
  default:
    break;
  }
...
{
  sprintf(Error_Text,"Couldn't create %s (not implemented)\n",name);
  Error(Error_Text);
