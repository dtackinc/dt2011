...
#define SYSTEM_START_CODE       0x1BB
    ...
    case SEQUENCE_HEADER_CODE:
	  UDEBUG(("next_start_code %03x SEQUENCE_HEADER_CODE\n",code));
      sequence_header();
	  ssg_need_lep = 0;					/* this will be our LEP 			*/
      break;
    case GROUP_START_CODE:
      group_of_pictures_header();
	  UDEBUG(("next_start_code %03x GROUP_START_CODE\n",code));
      break;
    case PICTURE_START_CODE:
	{
	  int skip = 0;
      picture_header();
	  if (ssg_need_lep != 0)			/* don't leave until we get a LEP 	*/
        skip = 1;
	  else
	  if (picture_coding_type >
		  ssg_keep_picture_coding)
	    skip = 1;
	  
	  UDEBUG(("next_start_code %03x PICTURE_START_CODE type=%d (%d) skip=%d\n",
        code, picture_coding_type, ssg_keep_picture_coding, skip));
    ...
    case SEQUENCE_END_CODE:
	  UDEBUG(("next_start_code %03x SEQUENCE_END_CODE\n",code));
      return 0;
      break;
    default:
      if (!Quiet_Flag &&
		  (code & 0xfffff1e0) != 0x00000100)
