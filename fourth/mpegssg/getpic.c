...
static int frame_reorder _ANSI_ARGS_((int bitstream_framenum, 
...
int Decode_Picture(bitstream_framenum, sequence_framenum)
int bitstream_framenum, sequence_framenum;
{
  ...
  rc = frame_reorder(bitstream_framenum, sequence_framenum);
...
static int frame_reorder(Bitstream_Framenum, Sequence_Framenum)
int Bitstream_Framenum, Sequence_Framenum;
{
  ...
  if (Sequence_Framenum!=0)
  {
    ...
    {
      if (picture_coding_type==B_TYPE)
        rc = Write_Frame(auxframe,Bitstream_Framenum-1);
      else
      {
        Newref_progressive_frame = progressive_frame;
        progressive_frame = Oldref_progressive_frame;

        rc = Write_Frame(forward_reference_frame,Bitstream_Framenum-1);
